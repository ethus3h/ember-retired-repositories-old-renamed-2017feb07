...
, mActiveWinId(0)
, mAppMenuImporter(new KAppMenuImporter())
, mMenuWidget(0)
{
...
void MenuBarApplet::setupDBusInterface()
{
    ...
    connect(mAppMenuImporter, SIGNAL(actionActivationRequested(WId, QAction *)),
            SLOT(slotActionActivationRequested(WId, QAction *)));

    connect(mAppMenuImporter, SIGNAL(windowUnregistered(WId)),
...
void MenuBarApplet::createMenuBar()
{
    ...
    QMenu* menu = mAppMenuImporter->menuForWinId(winId);
    ...
    if (!menu) {
        if (winId && !isDesktopWinId(winId)) {
            ...
            if (mainWinId) {
                ...
                QMenu* mainMenu = mAppMenuImporter->menuForWinId(mainWinId);
                if (mainMenu) {
...
void MenuBarApplet::slotActionActivationRequested(WId wid, QAction* action)
{
    if (mActiveWinId == wid) {
        if (!mMenuWidget) {
